...
(function ($) {
   // var geoNamesIdIgnoreList = [6697173,661882];
    $.fn.jeoCountrySelect = function (options) {
        ...
        jeoquery.countryInfo(function (data) {
            ...
            $(sortedNames).each(function () {
                ...
                if ($.inArray(this.geonameId, jeoquery['aExcludeGeoNameIds']) === -1) {
                   
                    //var item = $("<option></option>").attr("value", this.countryCode).text(this.countryName);
                    var item = $("<option></option>").attr("value", this.geonameId).text(this.countryName);
                    el.append(item);
                }
                else {
                    console.log('did not add ' + this.countryName );
                }
            });
